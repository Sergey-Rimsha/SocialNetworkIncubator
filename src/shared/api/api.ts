...
export const authApi = {
  authLoginMe() {
    return instance.get<ResponseType<AuthUserType>>(`auth/me`).then(res => {
      // console.log(res)
      return res.data;
    });
  },
...
export const usersApi = {
  usersPage(currentPage: number, userPageSize: number) {
    return instance.get(`users?page=${currentPage}&count=${userPageSize}`).then(response => {
  ...
  putUserStatus(status: string) {
    return instance.put<ResponseType<{}>>(`/profile/status`, { status }).then(res => {
      return res.data;
    });
  },
  /// *
...
export const profileApi = {
  ...
  updateProfile(profileData: UpdateProfileType) {
    return instance.put<ResponseType<{}>>('/profile', profileData).then(resolve => {
      return resolve.data;
    });
  },
};
